...
5. **Anti-hallucination check + grounding** — `find_span(page.text, quote)` locates the quote's exact character range on its page (tolerating line-break/whitespace differences); `boxes_for_span` turns that range into pixel rectangles stored as `quote_boxes`. **Finding the quote is the verification.** If it isn't on the page we fall back to the whole-document check (`_normalize(quote) in norm_full`) so a quote straddling a page break still verifies — it just carries no boxes. If it's nowhere at all, we still store the obligation but mark it **unverified** so the UI can flag it.
...
- **Verification is cheap and honest** — we locate the quote in the source rather than trusting it. It won't catch a cleverly paraphrased hallucination, but it *does* catch fabricated quotes, and it never blocks an item — it flags it. Reviewers make the final call (human-in-the-loop `PATCH` in the obligations route). The nice side effect: the same lookup that proves the quote is real also tells us where to draw the highlight.
