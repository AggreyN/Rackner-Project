...
**Likely question — "What about scanned PDFs with no text layer?"** → `get_text("words")` returns nothing for image-only pages, so `words` is empty and we fall back to `get_text()`. Of the 5 sample SAM.gov PDFs, `W50S8J26QA017.pdf` and `W912HN26RA012.pdf` are image-only and need OCR (pytesseract/pdf2image) — the planned **Week-7** fallback, dependencies listed (commented) in `requirements.txt`.
