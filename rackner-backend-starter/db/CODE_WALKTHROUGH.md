...
`document_id` (FK, `ondelete="CASCADE"`), `clause_ref` ("252.204-7012"), `text`, `page`, `char_start`, `char_end`, `boxes` (JSON — the word rectangles covering that span). The offsets and boxes are the citation trail from `segment.py`.
...
- Traceability & workflow: `source_clause_id` (FK), `page`, `quote_char_start` / `quote_char_end` / `quote_boxes` (exactly where the quote sits on the page — the span-highlight payload), `verified` (the anti-hallucination flag), `status` (`open|in-review|done`).
