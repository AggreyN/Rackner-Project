...
// Every real-backend call carries the JWT when the auth flag is on.
...
export async function scanDocument(file: File): Promise<ScanResult> {
  ...
  return json(await fetch(`${BASE}/documents/scan`, opts({ method: "POST", body: form })));
}
...
export async function uploadDocument(
  ...
  return json(await fetch(`${BASE}/documents`, opts({ method: "POST", body: form })));
}

export async function getDocument(id: number): Promise<DocumentMeta> {
  if (USE_MOCK) return mock.getDocument(id);
  return json(await fetch(`${BASE}/documents/${id}`, opts()));
}
...
export async function getRoles(): Promise<RoleInfo[]> {
  ...
  return json(await fetch(`${BASE}/obligations/roles`, opts()));
}
...
export async function getObligations(
  ...
  return json(await fetch(`${BASE}/obligations/document/${docId}?${params}`, opts()));
}
...
export async function updateObligationStatus(id: number, status: string): Promise<void> {
  ...
  await json(await fetch(`${BASE}/obligations/${id}?status=${status}`, opts({ method: "PATCH" })));
}
