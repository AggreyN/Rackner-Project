...
function HomeInner() {
  ...
  // Keep the input box in sync with the URL (Back/Forward, a new search)
  // without a cascading effect — React's sanctioned adjust-during-render.
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // The loading flags must flip synchronously with the fetch they describe —
    // deferring them would let the previous result render as if it were
    // current. Intentional, so the cascading-render rule is suppressed here.
